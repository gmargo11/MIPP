...
def explore(x_train, y_train, num_features):
    ...
    x_candidates = np.concatenate([generate_grid(res), np.ones((res*res, 1))*1], axis=1)
    novelties = compute_novelties(joint_distribution, x_train, x_candidates)
...
def follow_complete_policy():
    ...
    for i in range(31):
        if i > 24: #alpha <  np.random.uniform():
            print('exploit')
            ...
            if d_nextSample > 0.1:
                nextSample = np.array([(nextSample[0] - loc[0]) / d_nextSample * 0.1 + loc[0], (nextSample[1] - loc[1]) / d_nextSample * 0.1 + loc[1], nextSample[2]])
